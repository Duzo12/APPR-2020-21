...
source("lib/styles.sty", encoding="UTF-8")

# Uvoz, obdelava in ÄŤiĹˇÄŤenje podatkov
...
uvozizobrazba <- function(Izobrazba){
  ...
  Izobrazba <- Izobrazba %>% mutate(Country = substring(Country, 2)) #znebim se presledka, ki je bil pred vsakim imenom drĹľave
  ...
  Izobrazba[,-1] <- Izobrazba[,-1] * 100 #pomnoĹľim * 100, da so enaki index safety in criminal 
  return(Izobrazba)
}
